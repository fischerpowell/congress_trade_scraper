...
driver.find_element(By.XPATH, "//input[@class='report_types_annual form-check-input']").click()
...
while not done:
    ...
    for outer_table_row in outer_table_rows:    
        ...
        try:
            transaction_table = second_driver.find_element(By.XPATH, "//section[contains(., 'Part 4b. Transactions')]")
            inner_heading_row = [heading_value.text for heading_value in transaction_table.find_elements(By.XPATH, ".//thead//tr//th")]
            inner_table_rows = transaction_table.find_elements(By.XPATH, ".//tbody//tr[@class='nowrap']")
...
with open("senator_trading_dataset.json", "w") as jsonfile:
